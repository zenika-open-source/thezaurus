...
export default function Index() {
  ...
  const [diffusionMessageRead, setDiffusionMessageAsRead] = useLocalStorageOnce(
    false,
    "diffusionMessageRead"
  );
    ...
    return (
      <main className={`${opensans.className} flex flex-col h-full px-2`}>
        ...
        <header className="py-1 grid grid-cols-[1fr_repeat(1,auto)_1fr] items-center">
          ...
          <Link className="button button-nav" href="/api/auth/logout">
            <IconLogout width={30} />
          </Link>
        </header>
        ...
        <section className="description text-center text-sm mx-auto my-2 p-2 rounded-md">
          <details open={!diffusionMessageRead}>
            <summary className="text-lg mb" onClick={setDiffusionMessageAsRead}>
              👩‍⚖️ --{" "}
              {diffusionMessageRead
                ? "Rappel: diffusion interne uniquement"
                : "À lire attentivement avant utilisation"}{" "}
              -- 👨‍⚖️
            </summary>
            <div className="p-2">
              <p>
                Listing de support d&apos;upskilling pour les consultants
                Zenika. Ces médias sont des productions internes, et ne doivent
                pas être diffusées hors de Zenika sauf mention contraire
                explicite&nbsp;
                <IconPublic />. Chaque média est daté et le contexte de
                présentation est précisé ici.
              </p>
              <p>
                Sont éligibles ici les vidéos avec contenu de formation, avancé
                ou non, permettant aux consultants internes de découvrir ou
                d&apos;approfondir un sujet. Les contenus commerciaux ne sont
                pas la cible de cette librairie.
              </p>
              <p>
  ...
  return (
    <div className="h-full flex items-center justify-center">
      <Link
        href="/api/auth/login"
        className="button button-primary connexion h-10 w-40 rounded-lg px-4 py-2 inline-bloc text-center"
      >
        Login
      </Link>
    </div>
